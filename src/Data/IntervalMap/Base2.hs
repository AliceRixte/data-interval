...
newtype IntervalMap r a =
  IntervalMap (Maybe a, Map (r, Interval.Boundary) (Maybe a))
  deriving
    ( Eq
    , Ord
...
-- newtype LB r = LB (r, Interval.Boundary)
...
-- empty = IntervalMap (Nothing, Map.empty)
...
-- toList :: Ord k => IntervalMap k a -> [(Interval k, a)]
-- toList = toAscList

-- flipBoundary :: Interval.Boundary -> Interval.Boundary
-- flipBoundary Interval.Open = Interval.Closed
-- flipBoundary Interval.Closed = Interval.Open

-- flipBound :: (Extended r, Interval.Boundary) -> (Extended r, Interval.Boundary)
-- flipBound (r, b) = (r, flipBoundary b)

-- -- | Convert the map to a list of key/value pairs where the keys are in ascending order.
-- toAscList :: Ord k => IntervalMap k a -> [(Interval k, a)]
-- toAscList (IntervalMap (a,m)) =
--   case maxa of
--     Nothing -> l
--     Just a' ->
--       (Interval.interval maxlb (PosInf, Interval.Open), a') : l

--   where
--   (maxlb, maxa, l) = Map.foldrWithKey step ((NegInf, Interval.Open), a, []) m
--   step (LB nextlb) nexta (prevlb, preva, acc) =
--     let nextlb' = first Finite nextlb in
--     case preva of
--       Nothing -> (nextlb', nexta, acc)
--       Just preva' ->
--         ( nextlb'
--         , nexta
--         , (Interval.interval prevlb (flipBound nextlb'), preva') : acc
--         )
...
-- -- filterWithInterval :: Ord k
-- --   => (Interval k -> a -> Bool) -> IntervalMap k a -> IntervalMap k a
-- -- filterWithInterval p (IntervalMap m) =
-- --   IntervalMap $ Map.filter (\(k,a) -> p k a) m

-- -- -- | Filter all intervals that overlap with the given interval.
-- -- filterOverlapping :: Ord k
-- --   => Interval k -> IntervalMap k a -> IntervalMap k a
-- -- filterOverlapping i (IntervalMap m) =
-- --   IntervalMap $ Interval.restrictMapKeysToInterval
-- --                       (Interval.interval (lowerBound' i <=..<=
-- --                       m
-- --   where
-- --     (l, lb) = Interval.lowerBound' i
-- --     (u, ub) = Interval.upperBound' i
