...
withoutKeysFromInterval i m = case i of
  ...
  BothClosed lk uk -> let (lt,_,gt) = Map.splitLookup uk m in
    restrictKeysToInterval lt (LessThan lk) `Map.union` gt
  LeftOpen lk uk -> let (lt,_,gt) = Map.splitLookup uk m in
    restrictKeysToInterval lt (LessOrEqual lk) `Map.union` gt
  RightOpen lk uk -> let (lt,_,gt) = Map.splitLookup lk m in
    lt `Map.union` restrictKeysToInterval gt (GreaterOrEqual uk)
  BothOpen lk uk ->
    restrictKeysToInterval m (LessOrEqual lk)
    `Map.union` restrictKeysToInterval m (GreaterOrEqual uk)
{-# INLINE withoutKeysFromInterval #-}
