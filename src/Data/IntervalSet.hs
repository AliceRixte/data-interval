...
-- internal helper for intersectSetAndIntervalSet and differenceSetAndIntervalSet
intersectSetAndIntervalSetFold :: Ord k => Set k -> IntervalSet k -> Set k
intersectSetAndIntervalSetFold s is = foldr f Set.empty is
  where f i acc = Set.union acc (Interval.intersectSetAndInterval s i)
{-# INLINE intersectSetAndIntervalSetFold #-}

-- the ratio size of the set / size of the interval set when
-- intersectSetAndIntervalSetFold and Map.filterWithKey have similar performance.
foldFilterSetRatio :: Int
foldFilterSetRatio = 4

-- | Restrict a 'Set' to the keys contained in a given 'Interval'.
--
-- >>> intersectSetAndIntervalSet s i == filter (\k -> IntervalSet.member k i) s
--
-- [Usage:]
--
-- >>> s = Set.fromList [-2.5, 3.1, 5 , 8.5] :: Set Rational
-- >>> intersectSetAndIntervalSet s (IntervalSet.fromList [ -inf <..<= 3, 8 <..< inf])
-- fromList [(-5) % 2,17 % 2]
--
-- [Performance:] \(O(\min (n \log i, i + m \log m)\), with \(n\), the size of
...
intersectSetAndIntervalSet :: Ord k => Set k -> IntervalSet k -> Set k
intersectSetAndIntervalSet s is@(IntervalSet mi) =
  if foldFilterSetRatio * Map.size mi <= Set.size s then
    intersectSetAndIntervalSetFold s is
  else
    Set.filter (\k -> member k is) s
{-# INLINE intersectSetAndIntervalSet #-}
