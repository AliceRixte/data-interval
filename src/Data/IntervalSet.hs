...
module Data.IntervalSet
  ...
  , restrictKeysToIntervals
  , withoutKeysFromIntervals
...
-- internal helper for restrictKeysToIntervals and withoutKeysFromIntervals
restrictKeysToIntervalsFold :: Ord k => Map k a -> IntervalSet k -> Map k a
restrictKeysToIntervalsFold m is = foldr f Map.empty is
  where f i acc = Map.union acc (Interval.restrictKeysToInterval m i)
{-# INLINE restrictKeysToIntervalsFold #-}

-- the ratio size of the map / size of the interval set when
-- restrictKeysToIntervalsFold and Map.filterWithKey have similar performance.
foldFilterMapRatio :: Int
foldFilterMapRatio = 4

-- | Restrict a 'Map' to the keys contained in a given 'Interval'.
--
-- >>> restrictKeysToIntervals m i == filterKeys (\k -> IntervalSet.member k i) m
--
-- [Usage:]
--
-- >>> m = Map.fromList [(-2.5,0),(3.1,1),(5,2), (8.5,3)] :: Map Rational Int
-- >>> restrictKeysToIntervals m (IntervalSet.fromList [ -inf <..<= 3, 8 <..< inf])
...
--
restrictKeysToIntervals :: Ord k => Map k a -> IntervalSet k -> Map k a
restrictKeysToIntervals m is@(IntervalSet mi) =
  if foldFilterMapRatio * Map.size mi <= Map.size m then
    restrictKeysToIntervalsFold m is
  else
    -- faster when the interval set is large compared to the map
    Map.filterWithKey (\k _ -> member k is) m
{-# INLINE restrictKeysToIntervals #-}

------------------------------  Complexity proof -------------------------------
--
-- The performance of restrictKeysToIntervals is O(i + m log m) with m the size
...
--
-- >>> withoutKeysFromIntervals i m == filterKeys (\k -> not (IntervalSet.member k i)) m
--
-- [Usage:]
--
-- >>> m = Map.fromList [(-2.5,0),(3.1,1),(5,2), (8.5,3)] :: Map Rational Int
-- >>> withoutKeysFromIntervals (IntervalSet.fromList [ -inf <..<= 3, 8 <..< inf]) m
-- fromList [(31 % 10,1),(5 % 1,2)]
--
-- See performance note for 'restrictKeysToIntervals'.
withoutKeysFromIntervals :: Ord k => IntervalSet k -> Map k a -> Map k a
withoutKeysFromIntervals is@(IntervalSet mi) m =
  if foldFilterMapRatio * Map.size mi <= Map.size m then
    restrictKeysToIntervals m $ complement is
    -- This is faster in most cases than folding withoutKeysFromInterval. See
    -- benchmark.
  else
    Map.filterWithKey (\k _ -> notMember k is) m
{-# INLINE withoutKeysFromIntervals #-}

---------------------------------- Set.Set -----------------------------------

-- internal helper for intersectIntervals and differenceIntervals
intersectIntervalsFold :: Ord k => Set k -> IntervalSet k -> Set k
intersectIntervalsFold s is = foldr f Set.empty is
  where f i acc = Set.union acc (Interval.intersectInterval s i)
{-# INLINE intersectIntervalsFold #-}

-- the ratio size of the set / size of the interval set when
-- intersectIntervalsFold and Map.filterWithKey have similar performance.
foldFilterSetRatio :: Int
foldFilterSetRatio = 4

-- | Restrict a 'Set' to the keys contained in a given 'Interval'.
--
-- >>> intersectIntervals s i == filter (\k -> IntervalSet.member k i) s
--
-- [Usage:]
--
-- >>> s = Set.fromList [-2.5, 3.1, 5 , 8.5] :: Set Rational
-- >>> intersectIntervals s (IntervalSet.fromList [ -inf <..<= 3, 8 <..< inf])
-- fromList [(-5) % 2,17 % 2]
--
-- [Performance:] \(O(\min (n \log i, i + m \log m)\), with \(n\), the size of
...
intersectIntervals :: Ord k => Set k -> IntervalSet k -> Set k
intersectIntervals s is@(IntervalSet mi) =
  if foldFilterSetRatio * Map.size mi <= Set.size s then
    intersectIntervalsFold s is
  else
    Set.filter (\k -> member k is) s
{-# INLINE intersectIntervals #-}

-- | Delete keys contained in a given 'Interval' from a 'Set'.
--
-- >>> differenceIntervals i s == filter (\k -> not (Interval.member k i)) s
--
-- [Usage:]
--
-- >>> s = Set.fromList [-2.5, 3.1, 5 , 8.5] :: Set Rational
-- >>> differenceIntervals (3 <=..< 8.5) s
-- fromList [(-5) % 2,17 % 2]
--
-- See performance note for 'intersectIntervals'.
differenceIntervals :: Ord k => Set k -> IntervalSet k ->  Set k
differenceIntervals s is@(IntervalSet mi) =
  if foldFilterSetRatio * Map.size mi <= Set.size s then
    intersectIntervalsFold s (complement is)
  else
    Set.filter (\k -> notMember k is) s
{-# INLINE differenceIntervals #-}
