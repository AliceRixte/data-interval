...
-- same semantics as Interval.restrictMapKeysToInterval
restrictMapKeysToIntervalSplit :: Ord k => Map k a -> Interval k -> Map k a
restrictMapKeysToIntervalSplit m i =
  let (_, mid , _) = Interval.splitMapAtBoundariesOf i m in mid

withoutMapKeysFromIntervalSplit ::  Ord k => Interval k -> Map k a -> Map k a
withoutMapKeysFromIntervalSplit is m =
  let (lt, _, rt) = Interval.splitMapAtBoundariesOf is m in
    lt `Map.union` rt
...
benchRestrictMapKeysToInterval m i =
    ...
    , bench "restrictMapKeysToInterval (split)" $
      whnf (restrictMapKeysToIntervalSplit m) i
...
benchWithoutKeysFromInterval i m =
  ...
  , bench "withoutMapKeysFromInterval (split)" $
      whnf (withoutMapKeysFromIntervalSplit i) m
  , bench "Map.filterKeys" $
