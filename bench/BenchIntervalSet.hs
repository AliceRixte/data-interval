...
restrictMapKeysToIntervalSetSplit :: Ord k => Map k a -> IntervalSet k -> Map k a
restrictMapKeysToIntervalSetSplit m is = snd $ IntervalSet.foldr f (m, Map.empty) is
  where
  f i (lt, acc) =
    let (lti, mi, _) = Interval.splitMapAtBoundariesOf i lt in
...
-- same semantics as IntervalSet.intersectSetAndIntervalSet
intersectSetAndIntervalSetFoldr :: Ord k
  => Set k -> IntervalSet k -> Set k
intersectSetAndIntervalSetFoldr s is =
    let f i acc = Set.union acc (Interval.intersectSetAndInterval s i) in
...
benchRestrictMapKeysToIntervals b m i =
  ...
  if b then
    [bench "restrictMapKeysToIntervalSetSplit" $
      whnf (restrictMapKeysToIntervalSetSplit m) i]
  else []
...
benchRestrictMapKeysToIntervalsPercentage a =
  bgroup ("num of intervals / map size = " ++ show a) $
    ...
    -- only activate splitMapAtBoundariesOfs below the foldFilterThreshold
...
benchIntersectIntervals s is =
  [ bench "IntervalSet.intersectSetAndIntervalSet" $
      whnf (IntervalSet.intersectSetAndIntervalSet s) is
  , bench "intersectSetAndIntervalSet (foldr)" $
      whnf (intersectSetAndIntervalSetFoldr s) is
  ]
...
bgroupIntervalSet =
  bgroup "IntervalSet"
    [
      ...
      bgroup "intersectSetAndIntervalSet" $
